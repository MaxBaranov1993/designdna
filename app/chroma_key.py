...
def remove_chroma(image: str, color: str = "#00ff00", tolerance: int = 32,
    ...
    if not re.fullmatch(r"#[0-9a-fA-F]{6}", color or ""):
        raise ValueError("Enter the background color as #RRGGBB")
    if not 0 <= tolerance <= 160 or not 1 <= softness <= 100:
        raise ValueError("Tolerance: 0–160, softness: 1–100")
    ...
    if original.width * original.height > 4_194_304:
        raise ValueError("Use an image up to 4 megapixels for local background removal")
    ...
    if not np.any((pixels[:, :, 3] > 0) & (alpha < pixels[:, :, 3])):
        raise ValueError("Selected color not found at the edge. Adjust the color or tolerance")
    if not np.any(alpha > 0):
        raise ValueError("The mask removes the entire image. Reduce tolerance or choose another color")
    if despill:
