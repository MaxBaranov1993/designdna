...
def interaction_build(req: InteractionBuildReq):
    ...
    if not FEATURE_FLAGS.is_enabled("interactionRecorder"):
        return err(404, "Interaction Recorder disabled by feature flag.")
    base_ir = ensure_current_ir(req.base_ir)
    schema_errors = validate_ir(base_ir)
    if schema_errors:
        return err(422, "Base IR fails schema validation: " + "; ".join(schema_errors[:5]))
    try:
...
def interaction_replay(req: InteractionReplayReq):
    ...
    if replay_errors:
        return err(422, "Scene patch creates invalid IR: " + "; ".join(replay_errors[:5]))
...
def interaction_capture(req: InteractionCaptureReq):
    ...
    if not FEATURE_FLAGS.is_enabled("interactionRecorder"):
        return err(404, "Interaction Recorder disabled by feature flag.")
    if not req.mine:
        return err(403, "Confirm that you own the site or have permission to record it.")
    base_ir = ensure_current_ir(req.base_ir)
    schema_errors = validate_ir(base_ir)
    if schema_errors:
        return err(422, "Base IR fails schema validation: " + "; ".join(schema_errors[:5]))
