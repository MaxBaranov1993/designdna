...
def main() -> None:
    with sync_playwright() as playwright:
        ...
        page.click('.dna-editor [data-ir-sec="0"]')
        page.click('[data-act="intent-locks"]')
        check("lock dialog targets current selection", "выбранн." in page.locator(".fe-locks-kicker").inner_text())
        page.locator(".fe-locks-list label", has_text="Геометрия").click()
        page.locator(".fe-locks-list label", has_text="Внешний вид").click()
        page.click('[data-act="apply-intent-locks"]')
        locks = page.evaluate("(id) => window.GraphDev.node(id).data.ir.tree[0].constraints.intentLocks", edit_id)
        ...
        undone = page.evaluate("(id) => window.GraphDev.node(id).data.ir.tree[0].constraints?.intentLocks || []", edit_id)
