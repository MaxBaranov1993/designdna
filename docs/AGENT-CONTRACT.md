...
6. **Регулятор параллелизма** на провайдера с видимой очередью и повтором при лимитах.
7. **Персистентные `runs` и `llm_calls`** в SQLite с версией контракта, моделью, усилием, транспортом и путём к сырому промпту и ответу.
