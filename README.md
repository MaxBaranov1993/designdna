...
Редактор открывается на `/` (React Flow-сборка из `frontend/` в `app/static/flow/`).
`/flow` — равнозначный маршрут, а устаревший `/nodes` отвечает 307-редиректом на `/`.
...
| Воспроизведение со скриншота или сайта | ✅ нода Reproduce и `/api/reproduce`: VLM → Python-измерения → HTML → native screenshot → pixel diff по регионам; есть кэш повторных запросов. Алгоритм — [docs/PIXEL-PERFECT-ALGORITHM.md](docs/PIXEL-PERFECT-ALGORITHM.md) |
| Нодовый граф (Houdini-like) | ✅ React Flow v1: холст pan/zoom, меню по правому клику, провода `text`/`ir`/`tokens` с проверкой циклов, ноды Prompt/Reference/Generator/Edit/Mix/Clone/Reproduce/BlockParse/Reskin, автосейв + экспорт/импорт JSON |
...
Следующие шаги: ноды для **Quality Gate + Constraints**, Wedge, Timeline и opt-in локальная Taste Memory; сборка дизайн-дока из нод остаётся отдельной фазой.
...
| `POST /api/block-parse` | `{url, blocks?}` → детекция блоков страницы и параллельный clone каждого в Design IR; URL проходит SSRF-гард. |
| `POST /api/reskin` | `{ir, prompt, tokens?, mask}` → `{ir, log}`. Рестайл через Qwen с детерминированным merge-back: структура, геометрия и невыбранные категории маски остаются неизменными. |
| `POST /api/quality-gate` | `{ir, fix?}` → правила v1, нарушения и при `fix=true` детерминированные правки сетки/overflow. |
| `POST /api/constraints/check` | `{ir, constraints}` → проверка декларативных lock/range/enum/max_len-ограничений. |
| `POST /api/reproduce` | `{image | url, provider, regions?}` → воспроизведение скриншота/сайта, Design IR, HTML и pixel diff; повторные запросы кэшируются. |
| `GET /`, `GET /flow`, `/static/*` | Фронтенд: React Flow-редактор. `GET /nodes` перенаправляет на `/`. |

## UI-флоу (нодовый редактор)

1. **Правый клик по холсту** → создать одну из девяти нод: Промпт, Референс, Генератор, Редактор, Микс, Клон, Reproduce, BlockParse или Reskin. Перетаскивание — за заголовок; панорама — drag фона, зум — колесо.
2. **Провода**: тянуть с выходного порта на входной. Типы `text`/`ir`/`tokens` проверяются, циклы отклоняются, на один вход — один провод.
...
app/
  server.py          FastAPI + uvicorn, порт 8420; LLM-клиент импортируется из spike/run_test.py.
                     Маршруты страниц: / и /flow — нодовый редактор; /nodes → 307 на /
  ...
  static/
    ...
    flow/                 собранный React Flow-интерфейс нодового редактора (артефакт Vite)
...
Коротко: превью только десктопное (артборд по умолчанию 960px); Reproduce уже доступна в графе, но pixel-perfect пайплайн остаётся специализированным и не смешивается с обычной генерацией IR; refine по промпту доступен только как API, ноды для него пока нет; `/api/generate` не валидирует ответ модели на бэкенде (невалидный вариант так и покажется в миниатюре — осознанно: видеть и чинить, а не терять). Ограничения прежней версии — в [REPORT.md](REPORT.md).
