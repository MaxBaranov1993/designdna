...
import { isLockedNode } from "./locked";
  ...
  /** Текстовый стиль элемента: валидная typeRole или "" — тогда роль берётся по тегу. */
  function typeRoleOf(el) {
    const role = el && typeof el.typeRole === "string" ? el.typeRole : "";
    return TYPE_ROLES.includes(role) ? role : "";
  }
  /** Стиль без инлайновых кегля/интерлиньяжа/веса/разрядки: их задаёт роль. */
  ...
  function renderElementInner(el, uid, parentFree, parentFrame) {
    ...
    switch (el.type) {
      ...
      case "heading": {
        ...
        // Текстовый стиль (typeRole): класс роли вместо инлайновых кегля/веса —
        // правка роли в токенах меняет все заголовки с этой ролью разом.
        const role = typeRoleOf(el);
        const style = role ? styleWithoutTypeOverrides(el.style) : el.style;
        const css = [visualCss(style), alignCss, placed ? "position:relative" : "",
          style?.fontSmoothing === "antialiased" ? "will-change:opacity" : ""].filter(Boolean).join(";");
