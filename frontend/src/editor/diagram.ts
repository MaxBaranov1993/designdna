...
export function diagramSection(ir: IRObject, title: string, labels: string[]): Record<string, any> {
  ...
  if (steps.length < 2 || steps.length > 6 || steps.some(v => v.length > 120)) throw new Error("Enter 2 to 6 steps, up to 120 characters each");
  if (title.trim().length > 160) throw new Error("The title must be shorter than 160 characters");
  ...
  steps.forEach((label, i) => {
    ...
    children.push({type: "frame", name: `Step ${i + 1}`, frame: {x, y, width: cellWidth, height: cellHeight, layout: "free"}, children: [
    ...
    if (i < steps.length - 1) children.push({type: "text", name: `Connection ${i + 1} → ${i + 2}`, text: vertical ? "↓" : "→", align: "center",
