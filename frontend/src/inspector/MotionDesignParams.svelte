...
<div class="dna-insp-fields">
  ...
  <div class="dna-field">
    <div class="dna-field-cap">Task / standalone prompt</div>
    <textarea rows="4" value={data.prompt}
      placeholder={hasReference ? "For example: continue the video and add a smooth camera fly-through" : "Describe the scene, motion, camera, and constraints"}
      oninput={(event) => $flow.setNodeData(id, { prompt: event.currentTarget.value, plannedPrompt: "" })}></textarea>
  </div>
  <div class="dna-insp-row">
    <div class="dna-field">
      <div class="dna-field-cap">Planner</div>
    ...
    <div class="dna-field">
      <div class="dna-field-cap">Effort</div>
  ...
  <div class="dna-insp-row">
    <div class="dna-field">
      <div class="dna-field-cap">Duration</div>
    ...
    <div class="dna-field">
      <div class="dna-field-cap">Aspect ratio</div>
    ...
    <div class="dna-field">
      <div class="dna-field-cap">Quality</div>
  ...
  <div class="dna-field">
    <button class="dna-btn-ghost" disabled={busy} onclick={() => void $flow.planMotionDesign(id)}>{data.plannedPrompt ? "Refresh prompt" : "Prepare prompt"}</button>
  ...
  {#if data.job}
    <div class="dna-field">
      <div class="dna-field-cap">Seedance job</div>
      ...
      {#if !["completed", "failed", "cancelled", "expired"].includes(data.job.status)}
        <button class="dna-btn-ghost" disabled={busy} onclick={() => void $flow.refreshMotionDesign(id)}>Refresh status</button>
      {/if}
    </div>
  {/if}
</div>
