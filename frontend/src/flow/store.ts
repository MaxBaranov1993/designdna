...
async function qualityPassCycle(
  ...
  { minScore = 85, repair = true, signal }: { minScore?: number; repair?: boolean; signal?: AbortSignal } = {},
): Promise<QualityPassResp> {
  const request = { ir, brief, min_score: minScore, repair, rejudge: repair };
  const desktop = window.designDNA;
  if (!desktop) return api<QualityPassResp>("/api/quality-pass", request, { signal });
...
export const useFlowStore = createStore<FlowStoreState>()((set, get) => ({
  ...
  cancelRun: async (id) => {
    ...
    controller?.abort();
    // Десктоп: длинный воркер занят чужим для fetch запросом (chatRequest /
    // IPC) — обрыв сигнала только освобождает ноду, сам воркер рестартуем.
    const desktopCancel = window.designDNA?.api?.cancel;
    if (desktopCancel) await desktopCancel().catch(() => undefined);
  ...
  runGenerator: async (id) => {
    ...
    try {
      ...
      if (!desktop) {
        ...
        res = await api<GenerateResp>("/api/generate", request, { signal });
      } else {
      ...
      for (let i = 0; i < variants.length; i += 1) {
        ...
        try {
          ...
          if (isAbortError(qpError)) throw qpError;
  ...
  runQualityPass: async (id) => {
    ...
    try {
      ...
      const res = await qualityPassCycle(ir, data.brief, provider, "high",
        ...
        { minScore: data.minScore, repair: data.repair, signal });
